...
const AstigmatismConfigModal = ({ config, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const submitData = {
                ...
                right_cylinder: formData.right_cylinder.filter(v => v !== ''),
                right_axis: formData.right_axis.filter(v => v !== ''),
            ...
            if (hasUnitImageFiles) {
                ...
                if (response.success) {
                    toast.success(
                        response.message ||
                            (isEditingExisting
                                ? 'Astigmatism configuration updated successfully'
                                : 'Astigmatism configuration created successfully')
                    );
                } else {
                    toast.success(
                        isEditingExisting
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full border border-gray-200/50 max-h-[90vh] flex flex-col">
                <div className="flex items-center justify-between p-6 border-b border-gray-200 sticky top-0 bg-white/95 backdrop-blur-sm z-10 flex-shrink-0">
                    <h2 className="text-2xl font-extrabold bg-gradient-to-r from-gray-900 via-indigo-800 to-purple-800 bg-clip-text text-transparent">
                        {isEditingExisting ? 'Edit Astigmatism Configuration' : 'Add Astigmatism Configuration'}
                    </h2>
                    <button
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto flex-1">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                required
                                placeholder="e.g., Monthly Astigmatism Config"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={handleChange}
                                className="input-modern"
                                placeholder="e.g., Monthly Astigmatism"
                            />
                        </div>
                    ...
                    <div className="border-t pt-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            {renderArrayField('right_cylinder', 'Right Cylinder (CYL)', 'e.g. -0.75')}
                            {renderArrayField('right_axis', 'Right Axis (AX)', 'e.g. 10')}
                        </div>
                    </div>
...
export default AstigmatismConfigModal;
